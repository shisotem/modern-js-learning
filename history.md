...
- `パッケージ管理システムnpm`
    ...
    - **パッケージ管理システム**
        ...
        2. パッケージのインストール及び削除
        3. <b id="pm3">依存関係の解決</b> : パッケージに必要な別のパッケージを自動でインストールしたり、更新することができる。
...
- `ブラウザとモジュール（IIFE, AMD, Bower）`
    ...
    - ***BAD***
        ...
        - Bowerは、<a href="#pm3">パッケージ管理システム3.</a>の機能が貧弱で、パッケージの依存関係を最終的には手作業で定義する必要があった。また、同一ページ内で任意のパッケージの異なるバージョン（e.g. jQueryの1系と2系）を両方使いたい場合に、それが不可能だった。
        - 他にも欠点はたくさんあった。。。
